...
int32_t DeviceInfoPipewire::GetDeviceName(uint32_t deviceNumber,
  ...
  PipewireNode& node = pipewire_session_->nodes().at(deviceNumber);
...
int32_t DeviceInfoPipewire::CreateCapabilityMap(
  ...
  for (auto& node : pipewire_session_->nodes()) {
    ...
    memset(_lastUsedDeviceName, 0, _lastUsedDeviceNameLength);
    node.display_name().copy(_lastUsedDeviceName, _lastUsedDeviceNameLength);
  }
  return 0;
}
...
bool DeviceInfoPipewire::IsDeviceNameMatches(const char* name,
                                             const char* deviceUniqueIdUTF8) {
