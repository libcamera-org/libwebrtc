...
spa_pod* BuildFormat(spa_pod_builder* builder,
  ...
  spa_rectangle min_size = spa_rectangle{1280, 720};
...
int32_t VideoCaptureModulePipewire::StartCapture(
  ...
  RTC_LOG(LS_INFO) << "Creating new PipeWire stream for node " << node_id_;

  pw_thread_loop_lock(session_->pw_main_loop_);
  ...
  if (prefer_jpeg)
    params.push_back(BuildFormat(&builder, SPA_MEDIA_SUBTYPE_mjpg, width,
                                 height, frame_rate));
  params.push_back(
  ...
  pw_thread_loop_unlock(session_->pw_main_loop_);
  return 0;
}
...
void VideoCaptureModulePipewire::OnFormatChanged(const struct spa_pod* format) {
  ...
  RTC_LOG(LS_ERROR) << "Configured capture format = "
  ...
  /*
    if (media_subtype == SPA_MEDIA_SUBTYPE_raw) {
      ...
      auto stride = VideoFrame::RowBytes(VideoFrame::kYPlane,
                                         capture_format_.pixel_format, width);
      spa_pod_builder_add(&builder, SPA_PARAM_BUFFERS_stride,
    SPA_POD_Int(stride), 0);
    }
  */
...
void VideoCaptureModulePipewire::OnStreamStateChanged(
  ...
  RTC_LOG(LS_ERROR) << "PipeWire stream state change: "
...
void VideoCaptureModulePipewire::ProcessBuffers() {
  ...
  while (pw_buffer* buffer = pw_stream_dequeue_buffer(stream_)) {
    ...
    if (last_buffer)
      pw_stream_queue_buffer(stream_, last_buffer);
    last_buffer = buffer;
  }
